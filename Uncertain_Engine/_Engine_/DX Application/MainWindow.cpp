...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    //rc = { 0, 0, (rc.right / 2) + ( rc.right / 4), (rc.bottom / 2) + (rc.bottom / 4)};
    rc = { 0, 0, rc.right, rc.bottom };
...
LRESULT CALLBACK WndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_RBUTTONDOWN:
		SetCapture(g_hWnd);
		DXApp::OnMouseDown(wParam, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		break;
	...
	case WM_RBUTTONUP:
		ReleaseCapture();
        DXApp::OnMouseUp(wParam, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		break;

	case WM_MOUSEMOVE:
        DXApp::OnMouseMove(wParam, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		break;;
