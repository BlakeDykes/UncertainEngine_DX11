#include "Shader/ShaderObjects/SO_BlendedTile_1_3_1.h"
#include "GameObject/GameObject.h"
#include "Lighting/SceneLightManager.h"

SO_BlendedTile_1_3_1::SO_BlendedTile_1_3_1()
...
void SO_BlendedTile_1_3_1::SetToCurrent()
{
...
void SO_BlendedTile_1_3_1::UpdateData(const GameObject* pGO, const MaterialObject* pMaterial)
{
	this->poCB_WWInvMat->Set(*pGO->GetWorld(), *pMaterial->GetLightParams());
	this->poCB_WWInvMat->Update();
}

void SO_BlendedTile_1_3_1::ReceiveCamData(const Mat4& view, const Mat4& proj, const Vec3& pos)
{
	this->poCB_VPPos->Set(view, proj, pos);
	this->poCB_VPPos->Update();
}

void SO_BlendedTile_1_3_1::ReceiveLightData(const SceneLightObject& light)
{
	this->poCB_LightParams->Set(&light.GetPhongData());
	this->poCB_LightParams->Update();
}

void SO_BlendedTile_1_3_1::ReceiveLightData(const unsigned char& light)
{
	this->poCB_LightParams->Set(&light);
	this->poCB_LightParams->Update();
}

void SO_BlendedTile_1_3_1::ReceiveFogData(const FogParams& fog)
{
	this->poCB_FogParams->Update((const void*)&fog);
}

void SO_BlendedTile_1_3_1::GenerateCBuffers(ID3D11Device* device, ID3D11DeviceContext* context)
{
