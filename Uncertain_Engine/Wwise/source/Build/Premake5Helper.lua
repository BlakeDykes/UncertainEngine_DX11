...
function SetOutputDirectory(in_root, in_platformName, in_config, in_suffix, targettype)
	...
	if (in_platformName == "iOS" or in_platformName == "tvOS" or in_platformName == "visionOS") then
		targetdir (root .. in_platformName .. "_Xcode$(XCODE_VERSION_MAJOR)/" .. in_config .. "$(EFFECTIVE_PLATFORM_NAME)" ..  "/" .. targettype)
		objdir ("!" .. root .. in_platformName .. "_Xcode$(XCODE_VERSION_MAJOR)/" .. in_config .. "$(EFFECTIVE_PLATFORM_NAME)"..  "/obj")
	elseif (in_platformName == "Mac") then
		if _ACTION == "xcode4" then
			targetdir (root .. in_platformName .. "_Xcode$(XCODE_VERSION_MAJOR)/" .. in_config ..  "/" .. targettype)
			objdir ("!" .. root .. in_platformName .. "_Xcode$(XCODE_VERSION_MAJOR)/" .. in_config .. "/obj/$(PREMAKE4_BUILDTARGET_BASENAME)")
		else
	...
	elseif (in_platformName == "XboxOneGC" or in_platformName == "WinGC") then
	...
	elseif (in_platformName == "NX") then -- no VS version suffix for NX
		targetdir (root .. "$(Platform)_SDK$(NintendoSdkVersionMajor)" .. "/$(Configuration)/" .. targettype)
		objdir ("!" .. root .. "$(Platform)_SDK$(NintendoSdkVersionMajor)" .. "/$(Configuration)/obj/$(ProjectName)")
	...
	elseif (in_platformName == "PS4") then
		local sdkOutputSuffix = "_SDK$([System.IO.Path]::GetFileName('$(SCE_ORBIS_SDK_DIR)'))"
		targetdir (root .. in_platformName .. sdkOutputSuffix .. "/" .. in_config ..  "/" .. targettype)
		objdir ("!" .. root .. in_platformName .. sdkOutputSuffix .. "/" .. in_config .. "/obj/$(ProjectName)")
	elseif (in_platformName == "Linux" ) then
		...
		filter {in_config, "platforms:x32"}
			targetdir (root .. "Linux_x32/" .. in_config ..  "/" .. targettype)
			objdir ("!" .. root .. "Linux_x32/" .. in_config .. "/obj/$(PREMAKE4_BUILDTARGET_BASENAME)")
		filter {in_config, "platforms:x64 or x86_64_cross_windows"}
			targetdir (root .. "Linux_x64/" .. in_config ..  "/" .. targettype)
			objdir ("!" .. root .. "Linux_x64/" .. in_config .. "/obj/$(PREMAKE4_BUILDTARGET_BASENAME)")
		filter {in_config}
	...
	elseif (in_platformName == "PS5") then
		local sdkOutputSuffix = "_SDK$([System.IO.Path]::GetFileName('$(SCE_PROSPERO_SDK_DIR)'))"
		targetdir (root .. in_platformName .. sdkOutputSuffix .. "/" .. in_config ..  "/" .. targettype)
		objdir ("!" .. root .. in_platformName .. sdkOutputSuffix .. "/" .. in_config .. "/obj/$(ProjectName)")
...
function DisablePropSheetElements()
	if IsVisualStudio() then
		...
		-- Needed to disable Edit and Continue
		filter { "options:not addresssanitizer" }
			removeelements {
