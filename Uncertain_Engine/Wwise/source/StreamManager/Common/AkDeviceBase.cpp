...
{
	AddTask(in_pStmTask, m_listTasks);
}
...
void CAkDeviceBase::RemoveTask(CAkStmTask* in_pStmTask)
{
	...
	in_pStmTask->pNextLightItem = NULL;
}
...
AKRESULT CAkStmTask::Open(const AkFileOpenData& in_FileOpen, bool in_bAuto)
{
...
AKRESULT CAkStmTask::MoveToNextDevice()
{
	...
	m_pDevice = CAkStreamMgr::AddTask(this, uDeviceID);
	if (!m_pDevice)
	{
...
void CAkStmTask::HandleFileOpenResult(AkAsyncFileOpenData* in_pOpenData, AKRESULT in_eResult)
{
	...
	if (bFinished)
	{
		...
		// Let the subclass take over; 
		OnFileDeferredOpen(*m_pOpenData, in_eResult);
...
AKRESULT CAkStdStmBase::Open(const AkFileOpenData& in_FileOpen, bool /*in_bAuto*/)
{
	m_eStmStatus = AK_StmStatusPending;

	AKRESULT res = CAkStmTask::Open(in_FileOpen, false);
	if (res != AK_Success)
		SetStatus(AK_StmStatusError, res);
	...
	)
{
	...
	if (in_eIOResult == AK_Success)
	{
		...
		{
			// Still pending: do not change status but check for cancel state			
...
{
	AkAutoLock<CAkLock> updateStatus( m_lockStatus );
	UpdateTaskStatus( AK_Fail, false );
}
...
AKRESULT CAkAutoStmBase::Open(const AkFileOpenData& in_FileOpen, bool)
{
	if (in_FileOpen.pFlags)
		m_fileID = in_FileOpen.pFlags->uCacheID;
	return CAkStmTask::Open(in_FileOpen, true);
}
    ...
    )
{
	...
	//
	AkUInt32 uLoopEnd;
	if ( !IsFileOpened() || in_heuristics.uLoopEnd <= m_pFileDesc->iFileSize)
...
// Hack for current PCMEx codec implementation that relies on the location of the first in-memory block (read, but not consumed).
// The official GetPosition() now returns the position of the next GetBuffer, not the position of the first-held memory view of that file.
// See WG-62522
AkUInt64 CAkAutoStmBase::GetFirstBufferPosition()
{
	AkAutoLock<CAkLock> stmBufferGate(m_lockStatus);
	...
	if (!m_listBuffers.IsEmpty())
		...
		uCurPosition = m_uNextExpectedFilePosition;
    ...
    )
{
    ...
    if (m_uNextExpectedFilePosition % m_uLLBlockSize != 0 )
    {
        ...
        AKASSERT(m_uNextExpectedFilePosition >= 0 );
    }
...
AKRESULT CAkAutoStmBase::PostOpenHelper()
{
	...
	if (m_uLoopEnd > m_pFileDesc->iFileSize)
		m_uLoopEnd = (AkUInt32)m_pFileDesc->iFileSize;
...
void CAkAutoStmBase::OnFileDeferredOpen(const AkAsyncFileOpenData& in_OpenData, AKRESULT in_eResult)
{
	...
	if (in_OpenData.pFlags)
		m_fileID = in_OpenData.pFlags->uCacheID;

	//Unblock this stream
	UpdateTaskStatus(in_eResult, false);
