...
CAkDeviceBase* CAkStreamMgr::AddTask(CAkStmTask* in_pTask, AkDeviceID in_deviceID)
{
...
)
{
	...
	AKRESULT res = pNewStm->Open(in_FileOpen);
	if (res == AK_Success && in_bSyncOpen)
	{
		pNewStm->WaitForPendingOperation();
		res = pNewStm->GetResult();
	}
...
)
{
	...
	else
	{
		...
		if (pStruct && pStruct->AllocData())
		{
			...
			if (AK_Success == CreateAuto(AkFileOpenData(in_fileID, AK_OpenModeRead, in_pFSFlags), heuristics, NULL, pIAkStream, true))
			{
...
namespace AK
{
	namespace StreamMgr
	{
		AkUInt64 GetCurBufferPositionFromAutoStm(IAkAutoStream* in_pStream)
		{
			CAkAutoStmBase* pAutoStream = (CAkAutoStmBase*)in_pStream;
			return pAutoStream->GetFirstBufferPosition();
		}
	}
}