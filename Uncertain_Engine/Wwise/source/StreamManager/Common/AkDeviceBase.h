...
namespace StreamMgr
{
    ...
    public:
		...
		virtual AKRESULT Open(const AkFileOpenData& in_FileOpen, bool in_bAuto = false);
    ...
    public:
		...
		AKRESULT Open(const AkFileOpenData& in_FileOpen, bool in_bAuto = false) override;
	...
	protected:
		...
		void UpdateTaskStatus(
			AKRESULT	in_eIOResult,			// AK_Success if IO was successful, AK_Cancelled if IO was cancelled, AK_Fail otherwise.
			bool		in_hasPendingIO 		// True if any pending operation are linked to this task, false otherwise.
			);
    ...
    public:
		...
		AkUInt64 GetFirstBufferPosition();
