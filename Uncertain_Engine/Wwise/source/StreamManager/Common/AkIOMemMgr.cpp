	...
	)
{
	...
	if (uMemorySize > 0)
	{
		...
		AKRESULT res = m_StreamPool.Init(uMemorySize, uMaxBlockSize, uMinBlockSize, in_settings.ePoolAttributes, in_settings.pIOMemory);
		if (res != AK_Success)
			return res;
	}
