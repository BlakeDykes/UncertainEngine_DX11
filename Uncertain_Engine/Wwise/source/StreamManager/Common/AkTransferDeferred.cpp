...
// in order to determine whether IAkLowLevelIOHook::Cancel() will be called.
	...
	)
{
	...
	if (eResult != AK_Success)
	{
		...
		AK::Monitor::PostCodeVarArg(
			AK::Monitor::ErrorCode_IODevice,
			AK::Monitor::ErrorLevel_Error,
			AK::Monitor::MsgContext(),
			pLowLevelTransfer->m_pOwner->CurrentOp() == CAkStmTask::OpWrite ? "Write" : "Read", eResult);
#endif
	}
	...
	)
{
	if ( in_bCallLowLevelIO )
	{
		if ( !m_bWasLLIOCancelCalled )
		{
			...
			IAkLowLevelIOHook::BatchIoTransferItem pIoTransferItem[1];
