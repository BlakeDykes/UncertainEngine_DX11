...
void CAkDeviceDeferredLinedUp::Destroy()
{
	if (!AK::StreamMgr::UsingIOThread())
		ForceCleanup(false, AK_MAX_PRIORITY);
	CAkIOThread::Term();

	// Validation: after IO thread is complete, there should be no pending I/O requests.
...
void CAkStdStmDeferredLinedUp::Cancel()
{
	bool bWaitForCompletion = false;
	{
		...
		if (!m_listPendingXfers.IsEmpty() || !m_listCancelledXfers.IsEmpty())
		{
			...
			{
				// Last step should have cancelled everything but it is possible that some transfers are async and not completed yet
	...
	{
		// Wait if anything is still pending
	...
	// Cancel pending transfers if applicable.
	{
...

		{
			...
			while ( it != m_listCancelledXfers.End() )
			{
				...
				pTransfer->Cancel( 
					static_cast<IAkLowLevelIOHook*>( m_pDevice->GetLowLevelHook() ),
					bCallLowLevelIO, 
					bAllCancelled );
